        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($email) {
                ...
                if ($user instanceof Utilisateur) {
                    ...
                    // Envoyer uniquement le code par email
                    $resetService->sendResetCodeEmail($resetToken);
                }

                // Pour des raisons de sécurité, on affiche le même message
                $this->addFlash('success', '✅ Un code de réinitialisation a été envoyé si l\'adresse existe.');
    ...
    public function resetPasswordWithCode(
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($token) {
                ...
                if ($resetToken) {
                    $this->addFlash('success', '✅ Code valide. Veuillez saisir votre nouveau mot de passe.');
